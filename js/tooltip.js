...
ShowdownEnhancedTooltip.showPokemonTooltip = function(pokemon, pokemonData, isActive){
	...
	text += '<small>' + "Tier: " + Tools.getTemplate(pokemon.species).tier + '</small>';
	...
	if(moveData[pokemon.speciesid]){
		moves = moveData[pokemon.speciesid]["randomBattleMoves"];
		text += "<br><small>"+moves.join(", ")+"</small>"
	}
	text += '</h2>';
	
	// ================================== CUSTOM ====================================
	//Shamelessly stolen from a deprecated version of tooltips

	var cloneMyPokemon = {};
	cloneMyPokemon.stats = template.baseStats;
	cloneMyPokemon.baseAbility = template.baseAbility;
	cloneMyPokemon.item = template.item;
	cloneMyPokemon.hp = template.hp;
	cloneMyPokemon.maxhp = template.maxhp;

	if (isActive) {
		var modifiedStats = this.calculateModifiedStats(pokemon, cloneMyPokemon);
		var statsText = this.makeModifiedStatText(cloneMyPokemon, modifiedStats);
		if (statsText.match('<b')) {
			text += '<p style="opacity: 0.5"><small>HP:</small> '+template.baseStats.hp+
			...
			text += '<p><small>HP:</small> '+modifiedStats.hp+
			' <small>Atk:</small> '+modifiedStats.atk+
			' <small>Def:</small> '+modifiedStats.def+
			' <small>SpA:</small> '+modifiedStats.spa+
			' <small>SpD:</small> '+modifiedStats.spd+
			' <small>Spe:</small> '+modifiedStats.spe+'</p>';
		}else{
